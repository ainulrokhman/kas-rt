...
export default function JimpitanPage() {
  const [activeTab, setActiveTab] = useState<Tab>('TARIK');
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      setLoading(true);
      try {
        const wargadatas = await WargaRepository.getAll();
        setWargaList(wargadatas);
        
        const setting = await JimpitanRepository.getGlobalSetting();
  ...
  // Fetch Daily Transactions & Weekly Lokasi (when tanggalFilter changes)
  useEffect(() => {
  ...
  const loadDailyOperations = async () => {
    try {
      ...
      const txs = await JimpitanRepository.getTransactionsByDateBound(startOfDay, endOfDay);
      setDailyTransactions(txs);

      // Cari tau lokasi Tahlil untuk 'Minggu' ini
      const yMonth = `${parts[0]}-${parts[1]}`;
      const target = await JimpitanRepository.getMonthTarget(yMonth, true); // Auto create if not exist
      if (target) {
         setCurrentTarget(target);
         const mingG_ke = hitungMingguKeDariTanggal(tanggalFilter);
         const locs = target.lokasi_tahlil || {};
         // Kita coba reverse map: karena format lama bebas string, 
         // Jika lokasinya adalah salah satu warga, kita set.
         setLokasiTahlil(locs[mingG_ke] || '');
      }
    ...
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-1.5 flex relative shadow-inner max-w-md w-full">
         <button 
           ...
           className={`flex-1 flex flex-col items-center justify-center py-2.5 rounded-xl text-xs font-semibold transition-all duration-300 z-10 ${
             activeTab === 'TARIK' ? 'text-white' : 'text-slate-400 hover:text-slate-300'
           }`}
         >
         ...
         <button 
           ...
           className={`flex-1 flex flex-col items-center justify-center py-2.5 rounded-xl text-xs font-semibold transition-all duration-300 z-10 ${
             activeTab === 'LAPORAN' ? 'text-white' : 'text-slate-400 hover:text-slate-300'
           }`}
         >
         ...
         <button 
           ...
           className={`flex-1 flex flex-col items-center justify-center py-2.5 rounded-xl text-xs font-semibold transition-all duration-300 z-10 ${
             activeTab === 'SETTING' ? 'text-white' : 'text-slate-400 hover:text-slate-300'
           }`}
         >
      ...
      <div className="w-full">
        ...
        {activeTab === 'SETTING' && (
          <div className="animate-fade-in fade-in transition-all max-w-2xl">
             <SettingJimpitan 
